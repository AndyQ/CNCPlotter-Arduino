...
float StepsPerMillimeterX = 120.0;
float StepsPerMillimeterY = 120.0;

// Drawing robot limits, in mm
// OK to start with. Could go up to 50 mm if calibrated well.
// Changed to use home (0,0) as Center position.
float Xmin = -17;
float Xmax = 17;
float Ymin = -17;
float Ymax = 17;
...
void processIncomingLine( char* line, int charNB ) {
  ...
  while ( currentIndex < charNB ) {
    switch ( line[ currentIndex++ ] ) {              // Select command, if any
      case 'V':
        verbose = !verbose;
        Serial.print("Verbose logging is now ");
 ...
 **********************************/
void drawLine(float x1, float y1, int overrideLimits ) {
...
void home() {
  ...
  // set step delay to 0 to make homing faster
  int oldStepDelay = StepDelay;
  StepDelay = 0;
  
  // Force line to double x and y max (to make sure we get there as we could be in top right corner)
  drawLine( Xmin*2, Ymin*2, 1 );
  
  // Override Xpos and Ypos to set to -min rather than -min*2
  Xpos = Xmin * StepsPerMillimeterX;
  Ypos = Ymin * StepsPerMillimeterX;

  drawLine( 0, 0, 1 );

  // Set this as the current home position
  actuatorPos.x = 0;
  actuatorPos.y = 0;

  // Reset Step delay
  StepDelay = oldStepDelay;
